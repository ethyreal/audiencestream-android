...
import com.tealium.audiencestream.visitor.Processor;
...
final class ProfileRetriever implements
    ...
    ProfileRetriever(AudienceStream.Config config) {
        ...
        this.visitorProfileAddress = new Uri.Builder()
                ...
                .appendPath(AudienceStream.getVisitorId())
                .build().toString();
    }
    ...
    public void onDispatchSent(JSONObject data) {
        ...
        final boolean hasListeners = AudienceStream.getEventListeners().size() > 0;
    ...
    public void onHttpResponse(String url, String method, int status, String entity) {
        ...
        try {
            newProfile = Profile.fromJSON(entity);
        } catch (JSONException e) {
            Logger.e(e);
        }

        final Profile oldProfile = AudienceStream.getCachedProfile();
        final boolean isNotSameProfile = oldProfile == null || !oldProfile.equals(newProfile);

        if (isNotSameProfile && newProfile != null) {

            if (Constant.DEBUG) {
                Log.v(Constant.DEBUG_TAG, "! Detected profile update: " + newProfile.toString());
            }
            ...
            AudienceStream.setCachedProfile(newProfile);
